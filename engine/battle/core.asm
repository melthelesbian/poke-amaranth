...
; the player is neither thrashing about nor charging for an attack
	...
	and (1 << FRZ) ; [INFO] allow attack after wake from sleep
	jr nz, .selectEnemyMove ; if so, jump
...
.WakeUp
	ld hl, WokeUpText
	call PrintText
	jr .FrozenCheck ; [INFO] allow attack after wake from sleep
.sleepDone
	xor a
...
.MonHurtItselfOrFullyParalysed
	...
	and ~((1 << STORING_ENERGY) | (1 << THRASHING_ABOUT) | (1 << CHARGING_UP) | (1 << USING_TRAPPING_MOVE) | (1 << INVULNERABLE)) ; [INFO] fix invulnerability glitch)
	ld [hl], a
	ld a, [wPlayerMoveEffect]
	cp FLY_EFFECT
...
; if the move type matches the "attacking type" and one of the defender's types matches the "defending type"
	...
	ldh [hMultiplier], a
	; [INFO] START BUGFIX CODE
	; New code for fixing effectiveness reporting bug
	; [TODO) credit the tutorial properly
	and a  ; cp NO_EFFECT
	jr z, .gotMultiplier
...
.wokeUp
	ld hl, WokeUpText
	call PrintText
	jr .checkIfFrozen ; [INFO] allow attack after wake from sleep
.sleepDone
	xor a
	ld [wEnemyUsedMove], a
