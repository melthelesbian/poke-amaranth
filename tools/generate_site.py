...
def build(model, out):
    ...
    public_items = sorted(
        (i for i in model["items"] if i["kind"] == "item" and i["symbol"] not in ("NO_ITEM", "POKEDEX") and not i["symbol"].startswith("UNUSED") and not i["symbol"].endswith("BADGE") and not i["aliases"]),
        key=lambda item: integer(item["bag_sort_order"]) if item["bag_sort_order"] else float("inf"),
    )
